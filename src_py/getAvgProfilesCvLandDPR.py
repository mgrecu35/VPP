...
fs=glob.glob("/media/grecu/ExtraDrive1/subSet/2*201806*sub_rate*HDF5")
...
btopL=np.array(btopL)

datL=[]
zmL1=[]
cL1=[]

for i in range(2*68,176):
    a=np.nonzero(bzdL==i)
    b=np.nonzero(bcfL[a]>=82*2)
    c=np.nonzero(pTypeL[a][b]==2)
    d=np.nonzero(btopL[a][b][c]<=bzdL[a][b][c])
    zmL1.append(10*(0.1*zmLs[a][b][c][d]).mean(axis=0))
    cL1.append(len(b[0]))
...
plt.subplot(111)
f=plt.pcolormesh(range(2*68,176),range(40,176),\
                 zmL1.T,cmap='jet',vmin=0,vmax=4)
plt.title('Minimal clutter')
plt.ylim(172,100)

plt.figure(figsize=(10,8))
plt.suptitle("Convective Land DPR")
plt.subplot(121)
for i in range(0,40):
    plt.scatter(zmL1[i,:],range(40,176))
...
plt.subplot(122)
for i in range(0,40):
    plt.scatter(zmL1[i,:]/zmL1[i,2*68+i-40],-(2*68+i)+np.arange(40,176))
    plt.plot(zmL1[i,:]/zmL1[i,2*68+i-40],-(2*68+i)+np.arange(40,176))
...
plt.xlabel('')
plt.tight_layout()
plt.savefig("avg_cv_Profiles_Land_DPR.png")

import pickle
pickle.dump(zmL1,open("zmL1_Land_cv_DPR.pklz","wb"))
stop
#plt.figure()
